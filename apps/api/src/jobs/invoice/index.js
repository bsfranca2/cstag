...
export const importInvoice = async (job) => {
  logger.info(`Iniciando importação da planilha de fatura: ${JSON.stringify(job)}`);

  const { id, bucketName, objectName } = job.data;
  ...
  for await (const { trip } of tripsStream) {
    promises.push(
      sendToQueue(queues.tripAnalysis, { invoiceId: id, trip })
    );
    tripsTotal += 1;
  }
  ...
  promises.push(sendToQueue(queues.ticketAnalysis, { invoiceId: id }));
  await Promise.all(promises);

  logger.info(`Finalizado importação da planilha de fatura: ${JSON.stringify(job)}`);
};
