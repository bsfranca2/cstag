import { index, deleteByQuery, indicies } from '@cstag/elasticsearch';
...
export const saveAnalysisES = async (tenant, data) => {
  await index({
    index: indicies.tripAnalysis.name,
    id: `${tenant}${data.id}`,
    document: formatES(data),
    refresh: true,
  });
};

export const deleteAnalysisES = async (tenant, trip) => {
  await deleteByQuery({
    index: indicies.tripAnalysis.name,
    query: {
      bool: {
        filter: [{ term: { tenant } }, { term: { trip } }],
      },
    },
    refresh: true,
  });
